...
// (every frame, ~50/60Hz) -- exactly one SIDPLAY call per IRQ, always,
// with no attempt to correct for a PAL/NTSC tempo mismatch (see
// sid_music_init()'s own comment, banking.c, for why that was tried and
// deliberately dropped). NOT from foreground code any more. This is what
// makes music keep playing *during*
...
// JSR/bank-restore/RTS, crashing into the KERNAL's BRK handler.
{
    ...
    // measured against this tune specifically).
