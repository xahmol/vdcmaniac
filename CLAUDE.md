...
```
make            # build vdcexp.prg, boot sector, and a D81 disk image in build/standard/
...
The Makefile has several commented-out alternate build paths (`krill`-loader
and `flossiec`-fastload variants, `d64`/`d71` disk images) that are
work-in-progress and not part of the active `all` target — leave them
commented unless specifically asked to wire them up, and update the
`MAIN_SRCS` list in the Makefile whenever `src/main.c` starts `#include`-ing
another `include/*.h` (it's a plain file list, not auto-derived from
`#pragma compile` chains, so make won't know to rebuild otherwise).
